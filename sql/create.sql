...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `firefly`.`getActions` (`id` INT, `inputId` INT, `outputId` INT, `switchDisplayName` INT, `port` INT, `inputDisplayName` INT, `hexValue` INT, `actionType` INT, `outputDisplayName` INT, `outputName` INT, `json` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `firefly`.`getControllerInputs` (`controllerId` INT, `json` INT);

-- -----------------------------------------------------
-- Placeholder table for view `firefly`.`getControllerOutputs`
-- -----------------------------------------------------
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `firefly`.`getOutputs` (`id` INT, `controllerId` INT, `name` INT, `displayName` INT, `outputType` INT, `pin` INT, `port` INT, `position` INT, `enabled` INT, `amperage` INT, `breakerId` INT, `controllerDisplayName` INT, `json` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `firefly`.`getSwitchBootstraps` (`switchId` INT, `macAddress` INT, `deviceName` INT, `name` INT, `displayName` INT, `json` INT);
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`getActions` AS select `firefly`.`actions`.`id` AS `id`,`firefly`.`actions`.`inputId` AS `inputId`,`firefly`.`actions`.`outputId` AS `outputId`,`firefly`.`switches`.`displayName` AS `switchDisplayName`,(case when (`firefly`.`inputs`.`port` = 1) then 'A' when (`firefly`.`inputs`.`port` = 2) then 'B' when (`firefly`.`inputs`.`port` = 3) then 'C' when (`firefly`.`inputs`.`port` = 4) then 'D' when (`firefly`.`inputs`.`port` = 5) then 'E' when (`firefly`.`inputs`.`port` = 6) then 'F' end) AS `port`,`firefly`.`inputs`.`displayName` AS `inputDisplayName`,concat('#',`firefly`.`buttonColors`.`hexValue`) AS `hexValue`,`firefly`.`actions`.`actionType` AS `actionType`,`firefly`.`outputs`.`displayName` AS `outputDisplayName`,`firefly`.`outputs`.`name` AS `outputName`,json_object('id',`firefly`.`actions`.`id`,'inputId',`firefly`.`actions`.`inputId`,'switchDisplayName',`firefly`.`switches`.`displayName`,'port',(case when (`firefly`.`inputs`.`port` = 1) then 'A' when (`firefly`.`inputs`.`port` = 2) then 'B' when (`firefly`.`inputs`.`port` = 3) then 'C' when (`firefly`.`inputs`.`port` = 4) then 'D' when (`firefly`.`inputs`.`port` = 5) then 'E' when (`firefly`.`inputs`.`port` = 6) then 'F' end),'inputDisplayName',`firefly`.`inputs`.`displayName`,'hexValue',concat('#',`firefly`.`buttonColors`.`hexValue`),'outputId',`firefly`.`actions`.`outputId`,'outputName',`firefly`.`outputs`.`name`,'outputDisplayName',`firefly`.`outputs`.`displayName`,'actionType',`firefly`.`actions`.`actionType`) AS `json` from ((((`firefly`.`actions` join `firefly`.`inputs` on((`firefly`.`actions`.`inputId` = `firefly`.`inputs`.`id`))) join `firefly`.`outputs` on((`firefly`.`actions`.`outputId` = `firefly`.`outputs`.`id`))) join `firefly`.`switches` on((`firefly`.`inputs`.`switchId` = `firefly`.`switches`.`id`))) join `firefly`.`buttonColors` on((`firefly`.`inputs`.`colorId` = `firefly`.`buttonColors`.`id`))) order by `firefly`.`switches`.`displayName`,`firefly`.`inputs`.`port`,`firefly`.`outputs`.`displayName`;
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`getControllers` AS select `firefly`.`controllers`.`id` AS `id`,`FORMATMACADDRESS`(`firefly`.`controllers`.`macAddress`) AS `macAddress`,`firefly`.`controllers`.`name` AS `name`,`firefly`.`controllers`.`displayName` AS `displayName`,inet_ntoa(`firefly`.`controllers`.`ipAddress`) AS `ipAddress`,inet_ntoa(`firefly`.`controllers`.`subnet`) AS `subnet`,inet_ntoa(`firefly`.`controllers`.`dns`) AS `dns`,inet_ntoa(`firefly`.`controllers`.`gateway`) AS `gateway`,`GETMQTTUSERNAME`(`firefly`.`controllers`.`macAddress`) AS `mqttUsername`,`GETMQTTPASSWORD`(`firefly`.`controllers`.`macAddress`) AS `mqttPassword`,`firefly`.`controllers`.`hwVersion` AS `hwVersion`,cast(if((`firefly`.`controllers`.`mqttUsername` is null),'true','false') as json) AS `mqttUsernameDefault`,cast(if((`firefly`.`controllers`.`mqttPassword` is null),'true','false') as json) AS `mqttPasswordDefault`,json_object('id',`firefly`.`controllers`.`id`,'macAddress',`FORMATMACADDRESS`(`firefly`.`controllers`.`macAddress`),'name',`firefly`.`controllers`.`name`,'displayName',`firefly`.`controllers`.`displayName`,'ipAddress',inet_ntoa(`firefly`.`controllers`.`ipAddress`),'subnet',inet_ntoa(`firefly`.`controllers`.`subnet`),'dns',inet_ntoa(`firefly`.`controllers`.`dns`),'gateway',inet_ntoa(`firefly`.`controllers`.`gateway`),'mqttUsername',`GETMQTTUSERNAME`(`firefly`.`controllers`.`macAddress`),'mqttUsernameDefault',cast(if((`firefly`.`controllers`.`mqttUsername` is null),'true','false') as json),'mqttPassword',`GETMQTTPASSWORD`(`firefly`.`controllers`.`macAddress`),'mqttPasswordDefault',cast(if((`firefly`.`controllers`.`mqttPassword` is null),'true','false') as json),'hwVersion',`firefly`.`controllers`.`hwVersion`) AS `json` from `firefly`.`controllers` order by `firefly`.`controllers`.`displayName`;
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`getControllerInputs` AS select `firefly`.`switches`.`controllerId` AS `controllerId`,json_arrayagg(json_object('id',`firefly`.`inputs`.`id`,'name',concat(`firefly`.`switches`.`name`,'B',`firefly`.`inputs`.`port`),'pin',`firefly`.`inputs`.`pin`,'circuitType',`firefly`.`inputs`.`circuitType`,'broadcastOnStateChange',((0 <> `firefly`.`inputs`.`broadcastOnChange`) is true),'enabled',((0 <> `firefly`.`inputs`.`enabled`) is true),'actions',coalesce(`firefly`.`getInputActions`.`json`,json_array()))) AS `json` from ((`firefly`.`inputs` join `firefly`.`switches` on((`firefly`.`inputs`.`switchId` = `firefly`.`switches`.`id`))) left join `firefly`.`getInputActions` on((`firefly`.`getInputActions`.`inputId` = `firefly`.`inputs`.`id`))) group by `firefly`.`switches`.`controllerId`;
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`getInputs` AS select `firefly`.`inputs`.`id` AS `id`,`firefly`.`inputs`.`switchId` AS `switchId`,(case when (`firefly`.`inputs`.`port` = 1) then 'A' when (`firefly`.`inputs`.`port` = 2) then 'B' when (`firefly`.`inputs`.`port` = 3) then 'C' when (`firefly`.`inputs`.`port` = 4) then 'D' when (`firefly`.`inputs`.`port` = 5) then 'E' when (`firefly`.`inputs`.`port` = 6) then 'F' end) AS `port`,`firefly`.`inputs`.`pin` AS `pin`,`firefly`.`inputs`.`colorId` AS `colorId`,concat('#',`firefly`.`buttonColors`.`hexValue`) AS `hexValue`,`firefly`.`inputs`.`circuitType` AS `circuitType`,`firefly`.`inputs`.`displayName` AS `displayName`,`firefly`.`switches`.`displayName` AS `switchDisplayName`,`firefly`.`switches`.`port` AS `controllerPort`,`firefly`.`controllers`.`displayName` AS `controllerDisplayName`,if(`firefly`.`inputs`.`broadcastOnChange`,'true','false') AS `broadcastOnChange`,if(`firefly`.`inputs`.`enabled`,'true','false') AS `enabled`,json_object('id',`firefly`.`inputs`.`id`,'switchId',`firefly`.`inputs`.`switchId`,'switchDisplayName',`firefly`.`switches`.`displayName`,'controllerId',`firefly`.`controllers`.`id`,'controllerDisplayName',`firefly`.`controllers`.`displayName`,'controllerPort',`firefly`.`switches`.`port`,'port',(case when (`firefly`.`inputs`.`port` = 1) then 'A' when (`firefly`.`inputs`.`port` = 2) then 'B' when (`firefly`.`inputs`.`port` = 3) then 'C' when (`firefly`.`inputs`.`port` = 4) then 'D' when (`firefly`.`inputs`.`port` = 5) then 'E' when (`firefly`.`inputs`.`port` = 6) then 'F' end),'pin',`firefly`.`inputs`.`pin`,'colorId',`firefly`.`inputs`.`colorId`,'hexValue',concat('#',`firefly`.`buttonColors`.`hexValue`),'circuitType',`firefly`.`inputs`.`circuitType`,'displayName',`firefly`.`inputs`.`displayName`,'broadcastOnChange',cast(if(`firefly`.`inputs`.`broadcastOnChange`,'true','false') as json),'enabled',cast(if(`firefly`.`inputs`.`enabled`,'true','false') as json)) AS `json` from (((`firefly`.`inputs` join `firefly`.`switches` on((`firefly`.`inputs`.`switchId` = `firefly`.`switches`.`id`))) join `firefly`.`buttonColors` on((`firefly`.`inputs`.`colorId` = `firefly`.`buttonColors`.`id`))) join `firefly`.`controllers` on((`firefly`.`switches`.`controllerId` = `firefly`.`controllers`.`id`))) order by `firefly`.`switches`.`displayName`,`firefly`.`inputs`.`port`;
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`getOutputs` AS select `firefly`.`outputs`.`id` AS `id`,`firefly`.`outputs`.`controllerId` AS `controllerId`,`firefly`.`outputs`.`name` AS `name`,`firefly`.`outputs`.`displayName` AS `displayName`,`firefly`.`outputs`.`outputType` AS `outputType`,`firefly`.`outputs`.`pin` AS `pin`,`firefly`.`outputs`.`port` AS `port`,if((`firefly`.`outputs`.`outputType` = 'BINARY'),1,2) AS `position`,if(`firefly`.`outputs`.`enabled`,'TRUE','FALSE') AS `enabled`,`firefly`.`outputs`.`amperage` AS `amperage`,`firefly`.`outputs`.`breakerId` AS `breakerId`,`firefly`.`controllers`.`displayName` AS `controllerDisplayName`,json_object('id',`firefly`.`outputs`.`id`,'controllerId',`firefly`.`outputs`.`controllerId`,'controllerDisplayName',`firefly`.`controllers`.`displayName`,'name',`firefly`.`outputs`.`name`,'displayName',`firefly`.`outputs`.`displayName`,'outputType',`firefly`.`outputs`.`outputType`,'pin',`firefly`.`outputs`.`pin`,'port',`firefly`.`outputs`.`port`,'enabled',((0 <> `firefly`.`outputs`.`enabled`) is true),'amperage',`firefly`.`outputs`.`amperage`,'breakerId',`firefly`.`outputs`.`breakerId`) AS `json` from (`firefly`.`outputs` join `firefly`.`controllers` on((`firefly`.`outputs`.`controllerId` = `firefly`.`controllers`.`id`)));
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`getSwitchBootstraps` AS select `firefly`.`switches`.`id` AS `switchId`,`FORMATMACADDRESS`(`firefly`.`switches`.`macAddress`) AS `macAddress`,hex(`firefly`.`switches`.`macAddress`) AS `deviceName`,`firefly`.`switches`.`name` AS `name`,`firefly`.`switches`.`name` AS `displayName`,json_object('name',`firefly`.`switches`.`name`,'network',json_object('ssid',`GETSETTING`('wifiSSID'),'key',`GETSETTING`('wifiKey')),'mqtt',json_object('serverName',`GETSETTING`('mqttServer'),'port',`GETSETTING`('mqttPort'),'username',`GETMQTTUSERNAME`(`firefly`.`switches`.`macAddress`),'password',`GETMQTTPASSWORD`(`firefly`.`switches`.`macAddress`),'topics',json_object('event',`GETSETTING`('eventTopic'),'client',`GETSETTING`('clientTopic'))),'buttons',ifnull(`firefly`.`getSwitchButtons`.`json`,json_array())) AS `json` from (`firefly`.`switches` left join `firefly`.`getSwitchButtons` on((`firefly`.`getSwitchButtons`.`switchId` = `firefly`.`switches`.`id`)));
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`getSwitchButtons` AS select `firefly`.`inputs`.`switchId` AS `switchId`,json_arrayagg(json_object('name',concat(`switches`.`name`,'B',`firefly`.`inputs`.`port`),'port',(case when (`firefly`.`inputs`.`port` = 1) then 'A' when (`firefly`.`inputs`.`port` = 2) then 'B' when (`firefly`.`inputs`.`port` = 3) then 'C' when (`firefly`.`inputs`.`port` = 4) then 'D' when (`firefly`.`inputs`.`port` = 5) then 'E' when (`firefly`.`inputs`.`port` = 6) then 'F' end),'led',`getColorBrightnessNames`.`brightnessNames`)) AS `json` from (((`firefly`.`inputs` join `firefly`.`buttonColors` on((`firefly`.`buttonColors`.`id` = `firefly`.`inputs`.`colorId`))) join `firefly`.`getColorBrightnessNames` on((`firefly`.`inputs`.`colorId` = `getColorBrightnessNames`.`colorId`))) join `firefly`.`switches` on((`firefly`.`inputs`.`switchId` = `firefly`.`switches`.`id`))) group by `firefly`.`inputs`.`switchId`;
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`statBreakerUtilization` AS select `firefly`.`breakers`.`id` AS `id`,`firefly`.`breakers`.`name` AS `name`,`firefly`.`breakers`.`displayName` AS `displayName`,`firefly`.`breakers`.`amperage` AS `amperage`,ifnull(`u`.`amperageUsed`,0) AS `amperageUsed`,ifnull(round(((`u`.`amperageUsed` / `firefly`.`breakers`.`amperage`) * 100),2),0) AS `utilization`,json_object('id',`firefly`.`breakers`.`id`,'name',`firefly`.`breakers`.`name`,'displayName',`firefly`.`breakers`.`displayName`,'amperage',`firefly`.`breakers`.`amperage`,'amperageUsed',ifnull(`u`.`amperageUsed`,0),'utilization',ifnull(round(((`u`.`amperageUsed` / `firefly`.`breakers`.`amperage`) * 100),2),0)) AS `json` from (`firefly`.`breakers` left join (select sum(`firefly`.`outputs`.`amperage`) AS `amperageUsed`,`firefly`.`outputs`.`breakerId` AS `breakerId` from `firefly`.`outputs` group by `firefly`.`outputs`.`breakerId`) `u` on((`u`.`breakerId` = `firefly`.`breakers`.`id`))) order by `firefly`.`breakers`.`displayName`;
