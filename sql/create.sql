...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `firefly`.`getActions` (`id` INT, `inputId` INT, `outputId` INT, `switchDisplayName` INT, `port` INT, `inputDisplayName` INT, `hexValue` INT, `actionType` INT, `outputDisplayName` INT, `json` INT);
...
USE `firefly`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `firefly`.`getActions` AS select `firefly`.`actions`.`id` AS `id`,`firefly`.`actions`.`inputId` AS `inputId`,`firefly`.`actions`.`outputId` AS `outputId`,`firefly`.`switches`.`displayName` AS `switchDisplayName`,(case when (`firefly`.`inputs`.`port` = 1) then 'A' when (`firefly`.`inputs`.`port` = 2) then 'B' when (`firefly`.`inputs`.`port` = 3) then 'C' when (`firefly`.`inputs`.`port` = 4) then 'D' when (`firefly`.`inputs`.`port` = 5) then 'E' when (`firefly`.`inputs`.`port` = 6) then 'F' end) AS `port`,`firefly`.`inputs`.`displayName` AS `inputDisplayName`,concat('#',`firefly`.`buttonColors`.`hexValue`) AS `hexValue`,`firefly`.`actions`.`actionType` AS `actionType`,`firefly`.`outputs`.`displayName` AS `outputDisplayName`,json_object('id',`firefly`.`actions`.`id`,'inputId',`firefly`.`actions`.`inputId`,'switchDisplayName',`firefly`.`switches`.`displayName`,'port',(case when (`firefly`.`inputs`.`port` = 1) then 'A' when (`firefly`.`inputs`.`port` = 2) then 'B' when (`firefly`.`inputs`.`port` = 3) then 'C' when (`firefly`.`inputs`.`port` = 4) then 'D' when (`firefly`.`inputs`.`port` = 5) then 'E' when (`firefly`.`inputs`.`port` = 6) then 'F' end),'inputDisplayName',`firefly`.`inputs`.`displayName`,'hexValue',concat('#',`firefly`.`buttonColors`.`hexValue`),'outputId',`firefly`.`actions`.`outputId`,'outputDisplayName',`firefly`.`outputs`.`displayName`,'actionType',`firefly`.`actions`.`actionType`) AS `json` from ((((`firefly`.`actions` join `firefly`.`inputs` on((`firefly`.`actions`.`inputId` = `firefly`.`inputs`.`id`))) join `firefly`.`outputs` on((`firefly`.`actions`.`outputId` = `firefly`.`outputs`.`id`))) join `firefly`.`switches` on((`firefly`.`inputs`.`switchId` = `firefly`.`switches`.`id`))) join `firefly`.`buttonColors` on((`firefly`.`inputs`.`colorId` = `firefly`.`buttonColors`.`id`))) order by `firefly`.`switches`.`displayName`,`firefly`.`inputs`.`port`,`firefly`.`outputs`.`displayName`;
